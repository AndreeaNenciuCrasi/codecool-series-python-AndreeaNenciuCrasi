...
def get_show_data(show_id):
    return data_manager.execute_select(f"SELECT shows.title, shows.year, shows.runtime, shows.trailer, shows.overview, string_agg(DISTINCT genres.name, ',') AS genres FROM shows LEFT JOIN show_genres ON shows.id = show_genres.show_id LEFT JOIN genres ON show_genres.genre_id = genres.id WHERE shows.id={show_id} "
...
def get_show_actors(show_id):
    return data_manager.execute_select(f"SELECT shows.id, show_characters.character_name, actors.name FROM shows "
                                       ...
                                       f"LEFT JOIN actors ON show_characters.actor_id = actors.id WHERE shows.id={show_id};")

def get_seasons(show_id):
    return data_manager.execute_select(f"SELECT shows.id, seasons.season_number, seasons.title, seasons.overview, COUNT(episodes.episode_number) AS episodesNumber FROM shows LEFT JOIN seasons ON shows.id = seasons.show_id LEFT JOIN episodes ON seasons.id = episodes.season_id WHERE shows.id={show_id} GROUP BY shows.id, seasons.season_number, seasons.title, seasons.overview;")
